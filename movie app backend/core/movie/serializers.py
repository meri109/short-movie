...
class MovieSerializer(serializers.ModelSerializer):
    ...
    # genre = serializers.CharField(source = 'genres.title', read_only = True) my ser
    ...
    def validate_title(self, value):
        if not value[0].istitle():
            raise serializers.ValidationError('Title should be capitalized')
        return value

    def validate(self, data):
        if "title" in data and "description" in data and data['title'].lower() in data['description'].lower():
            raise serializers.ValidationError('Title should not be used in description')
        return data
